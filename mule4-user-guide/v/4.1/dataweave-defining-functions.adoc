...
Define your own DataWeave functions (custom functions) using the `fun` declaration in the header of a DataWeave script. For example, this a simple DataWeave function accepts a single String argument that outputs `"HELLO"`:
...
To test the type of the argument passed into your function, you can link:dataweave-pattern-matching[match patterns] with the built-in `match` operation.
