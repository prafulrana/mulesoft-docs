...
DataWeave functions operate on many data types, including arrays, Booleans, objects, dates, and times. The types that DataWeave provide are bundled into modules that also contain the most closely related functions. 

Data types in the `dw::Core` module (Core types) are available without having to import the Core module. Other modules need to be imported for their functions and types to be available.
