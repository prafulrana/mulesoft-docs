...
This way, the default value for the `env` property is `"dev"`, which can still be overriden with a system or environment property. Note that this is *required* for metadata resolution in Anypoint Studio. If you do not define default values for the properties that are passed through the command line you receive an error while creating the application model for all message processors that depend on them.
