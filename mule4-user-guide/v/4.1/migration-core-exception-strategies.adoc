...
IMPORTANT: Examples do not cover the migration of expressions used within the `when` attribute.
See link:migration-mel[Migrating MEL to DataWeave] for a guide on expression migration
and link:connectors/v/latest/java-throwable[Working With Throwables] to learn how to use the Java module
