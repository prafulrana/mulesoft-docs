...
You can override this configuration at the domain or application level. You can create a custom file at a custom folder location and point to it in the link:intro-packaging#app_descriptor[application descriptor]. To do this, open the application's `mule-artifact.json` file in the root of the project and add a `logConfigFile` property with this location specified in it. The path to the file can be either relative to the working directory (the folder where the runtime was started from) or an absolute path.
...
By default, Mule polls modified configuration files every 60 seconds to check for changes. If any of those files have changed, the logger configuration is modified on the fly. You can customize this interval by setting the `monitorInterval` attribute in the root element (check link:https://logging.apache.org/log4j/2.x/manual/[log4j 2 manual] for further reference).
