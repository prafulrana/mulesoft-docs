...
You can override this config at the domain or application level. You can create a custom file at a custom folder location and point to it in the link:/mule-user-guide/v/4.1/intro-packaging#app_descriptor[application descriptor]. To do this, open the application's `mule-artifact.json` file in the root of the project and add a `logConfigFile` property with this location specified in it. The path to the file can be either relative to the Working Directory (the folder where the runtime was started from) or an absolute path.
